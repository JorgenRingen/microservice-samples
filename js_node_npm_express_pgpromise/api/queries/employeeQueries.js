...
exports.findAll = function (resultHandler, errorHandler) {
    return db.any('select id, firstname, lastname, date_of_birth from employee')
        .then(employeeRows => {
            const employees = employeeRows.map(employeeRow => mapEmployeeRowToEmployee(employeeRow));
...
exports.findById = function (employeeId, resultHandler, errorHandler) {
    return db.one('select id, firstname, lastname, date_of_birth from employee where id = $1', employeeId)
        .then(employeeRow => {
            resultHandler(mapEmployeeRowToEmployee(employeeRow));
        })
        .catch(error => {
            if (error.received === 0) {
                resultHandler(null);
...
function mapEmployeeRowToEmployee(employeeRow) {
    const dateOfBirth = (employeeRow.date_of_birth) ? moment(employeeRow.date_of_birth).format('YYYY-MM-DD') : null;
    return new employeeModel(employeeRow.id, employeeRow.firstname, employeeRow.lastname, dateOfBirth);
}
